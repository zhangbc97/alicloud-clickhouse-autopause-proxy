...
# COPY go1.21.4.linux-amd64.tar.gz go1.21.4.linux-amd64.tar.gz
...
RUN go build -buildmode=c-shared -v -o autopause_http.so

FROM envoyproxy/envoy:contrib-v1.28.0

COPY --from=build-network /go/src/app/autopause_http.so /lib/autopause_http.so
